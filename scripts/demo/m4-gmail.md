...
auth kind `imap-password` and run `wakewire auth imap` when instructed. The
...
1. In Google Cloud Console create an OAuth client, type **Desktop app** (any project;
   ...
   You bring your own client because wakewire is self-hosted — publishing a shared
...
2. In a Codex conversation:
   ...
   The model calls `wakewire_source_setup_gmail` with `{label: "agent-inbox", user: "you@gmail.com"}`
   and relays the instructions.

3. In a terminal: `wakewire auth gmail` — paste the client id/secret, approve in
   the browser. The daemon starts the IMAP IDLE watch immediately.

4. Create the route (the model does this via `wakewire_route_add` with
...
5. Send yourself an email and apply the `agent-inbox` label. Within ~30 seconds a
   `[wakewire event]` turn appears with the trimmed message (HTML converted to
