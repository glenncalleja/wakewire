...
Preset configs for the generic webhook source (`bridge_source_setup_webhook`).
...
1. `bridge_source_setup_webhook` with just `name` + `verification` — the next
   3 events are captured raw.
2. Trigger a test event from the provider.
3. `bridge_source_captures` — read the real payload.
4. Re-run `bridge_source_setup_webhook` with the mapping you authored.
5. `bridge_route_add` with `source: "webhook"`, `match: {"provider": "<name>"}`.
